...
{/if}
	{if isset($iForumId)}
		<div class="table">
			...
			<div class="table_right">
				<input placeholder="{phrase var='forum.title'}" class="form-control" type="text" name="val[title]" value="{value type='input' id='title'}" size="40" id="title" />	
			</div>
		</div>
	{/if}
	<div class="table">
	...
	{if Phpfox::isMobile()}
		<div class="table_clear">
			...
			<input type="submit" value="{if isset($iEditId)}{phrase var='forum.update'}{else}{phrase var='forum.submit'}{/if}" class="btn btn-primary btn-block" />
		</div>		
	{/if}
	<div{if Phpfox::isMobile()} id="js_mobile_form_holder" style="display:none;"{/if}>
		{if !isset($iEditId)}
			<div class="table">
				<div class="table_left">
					<label>{phrase var='forum.subscribe'}</label>
				</div>
				<div class="table_right">
					<div class="btn-group" data-toggle="buttons">
					  <label class="btn btn-default active">
							<input type="radio" name="val[is_subscribed]" value="1" class="v_middle"{value type='radio' id='is_subscribed' default='1' selected='true'}/> 
							{phrase var='forum.yes'}
					  </label>
					  <label class="btn btn-default">
							<input type="radio" name="val[is_subscribed]" value="0" class="v_middle"{value type='radio' id='is_subscribed' default='0'}/> 
							{phrase var='forum.no'}
					  </label>
					</div>			
				</div>
		...
		{if isset($iForumId) && $iForumId > 0 && Phpfox::isModule('poll') && Phpfox::getUserParam('poll.can_create_poll') && Phpfox::getUserParam('forum.can_add_poll_to_forum_thread')}	
			...
			<div class="table">
				...
				{if $bIsEdit && $aForms.poll_id > 0}
					{$aForms.poll_question|clean} - 
					<a href="#" onclick="$.ajaxCall('forum.deletePoll', 'poll_id={$aForms.poll_id}&amp;thread_id={$aForms.thread_id}'); return false;" title="{phrase var='forum.click_to_delete_this_poll'}">
						{phrase var='forum.delete'}
					</a>
				{/if}
				</div>
				<div id="js_attach_poll"{if $bIsEdit && $aForms.poll_id > 0} style="display:none;"{/if}>
					<input type="button" name="poll" value="{phrase var='forum.attach_poll'}" class="btn btn-default" onclick="tb_show('{phrase var='forum.attach_poll'}', $.ajaxBox('poll.add', 'height=340&amp;width=550&amp;item_id={$iForumId}&amp;module_id=forum'));" />	
				</div>
				</div>
			</div>		
		{/if}	
		...
		<div class="table_clear" id="js_forum_submit_button">
			<ul class="table_clear_button">
				<li><input type="submit" value="{if isset($iEditId)}{phrase var='forum.update'}{else}{phrase var='forum.submit'}{/if}" class="btn btn-primary" /></li>
		...
		{if (($bIsEdit && $aForms.is_announcement != 1) || (!$bIsEdit))}
			...
			{if ($bIsEdit && $aForms.is_announcement != 1) || (!$bIsEdit)}
				<div class="table">
					<div class="table_left">
						<label>{phrase var='forum.type'}</label>
					</div>
					<div class="table_right label_hover">			
						<select placeholder="{phrase var='forum.type'}" name="val[type_id]" class="form-control" {if Phpfox::getUserParam('forum.can_post_announcement') || Phpfox::getService('forum.moderate')->hasAccess('' . $iActualForumId . '', 'post_announcement')} onchange="selectThreadType(this);"{/if}>
		...
		<div class="table" id="js_forum_close">
			<div class="table_left">
				<label>{phrase var='forum.closed'}</label>
			</div>

			<div class="table_right">
				<div class="btn-group" data-toggle="buttons">
				  <label class="btn btn-default active">
						<input type="radio" name="val[is_closed]" value="1" class="v_middle"{value type='radio' id='is_closed' default='1'}/> 
						{phrase var='forum.yes'}
				  </label>
				  <label class="btn btn-default">
						<input type="radio" name="val[is_closed]" value="0" class="v_middle"{value type='radio' id='is_closed' default='0' selected='true'}/> 
						{phrase var='forum.no'}
				  </label>
				</div>			
			</div>
	...
	<div class="table_clear">
		<input type="submit" value="{if isset($iEditId)}{phrase var='forum.update'}{else}{phrase var='forum.submit'}{/if}" class="btn btn-primary btn-block mac-mrg-tp" />
	</div>
	{/if}
	{/if}
	{/if}
