...
{if (($aItem.user_id == Phpfox::getUserId() && Phpfox::getUserParam('blog.can_delete_own_blog_category')) || Phpfox::getUserParam('blog.can_delete_other_blog_category')) && $aItem.user_id != 0}
	...
	<div class="clear"></div>
{else}
	<input value="{$aItem.category_id}" type="checkbox" name="val[category][]" id="js_category{$aItem.category_id}" class="checkbox v_middle"/> {$aItem.name|convert|clean}
{/if}
</label>

</div>
<script type="text/javascript">
	$Behavior.loadBlogSelectCategories = function(){l}
