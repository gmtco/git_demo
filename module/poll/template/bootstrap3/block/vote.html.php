...
<div id="vote_{$aPoll.poll_id}">
	...
	<form method="post" action="{url link='current'}" id="js_poll_form_{$aPoll.poll_id}">
		...
		<div class="p_2">
			<label>
			<input class="js_poll_answer{if isset($iKey)}_{$iKey}{/if}" type="radio" name="val[answer]" value="{$answer.answer_id}" style="vertical-align:middle;" />
		...
		{/foreach}
		{/if}
		<div class="p_4" id="js_submit_vote{if isset($iKey)}_{$iKey}{/if}">
			...
			<input type="button" value="{phrase var='poll.submit_your_vote'}" class="btn btn-primary" onclick="$(this).parent().hide(); $(this).parents('.p_4:first').find('.js_poll_image_ajax:first').show(); $('#js_poll_form_{$aPoll.poll_id}').ajaxCall('poll.addVote');return false;" />
		</div>
