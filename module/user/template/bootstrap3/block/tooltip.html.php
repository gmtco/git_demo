<div class="user_tooltip_image">
	{img user=$aUser suffix='_50_square' max_width=50 max_height=50 class='mac-round-pic'}
</div>
...
<div class="user_tooltip_action">
	<ul>
		{if !$aUser.is_friend}
		<li><a class="btn btn-default btn-xs" href="#" onclick="return $Core.addAsFriend('{$aUser.user_id}');" title="{phrase var='profile.add_to_friends'}">{phrase var='user.add_as_friend'}</a></li>
		{/if}
		<li class="mac-mrg-tp"><a class="btn btn-default btn-xs" href="#" onclick="$Core.composeMessage({left_curly}user_id: {$aUser.user_id}{right_curly}); return false;">{phrase var='user.send_message'}</a></li>
		{if $bShowBDay == true}
			<li class="mac-mrg-tp"><a class="btn btn-default btn-xs" href="{url link=$aUser.user_name}">{phrase var='user.say_happy_birthday'}</a></li>
		{/if}
	</ul>
</div>
{/if}