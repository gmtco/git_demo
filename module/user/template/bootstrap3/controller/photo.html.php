			...
			{if ($bIsProcess) || (!$bIsProcess && $iImageWidth > 75 && $iImageHeight > 75)}
				...
				{if ($bIsProcess && ($iImageWidth <= 75 || $iImageHeight <= 75))}
					...
					<form method="post" action="#" onsubmit="return $Core.photo_crop.save(this);">
						...
						<input type="submit" value="{phrase var='user.save_avatar'}" id="js_save_profile_photo" class="btn btn-default" /> <span id="js_photo_preview_ajax"></span>		
					</form>
				{else}
					...
					<div class="p_4">
						...
						{/if}		
							...
							<div style="margin-top:10px;">
								<input type="submit" value="{phrase var='user.save_avatar'}" id="js_save_profile_photo" class="btn btn-primary" /> <span id="js_photo_preview_ajax"></span>
							</div>
						</form>			
					</div>
				{/if}
		...
		<div id="js_photo_form_holder">
			...
			<form method="post" action="{if $bIsRegistration}{url link='user.photo.register'}{else}{url link='user.photo'}{/if}" id="js_form" enctype="multipart/form-data" onsubmit="return startProcess(true, true);">		
				...
				<div class="table_clear">
					<input type="submit" value="{phrase var='user.upload_picture'}" class="btn btn-primary btn-block" name="val[uploaded]" />
				</div>		
			</form>
		</div>
	</div>	
