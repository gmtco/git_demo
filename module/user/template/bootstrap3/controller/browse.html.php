...
<div class="hidden-lg">
	...
	<div class="col-lg-12">														
		<div class="header_bar_search">
			<form class="mac-form-search-user-mobile form-horizontal" method="post" action="{url link='user.browse' view=$sView}">
				<div><input type="hidden" name="search[submit]" value="1" /></div>
				<div class="input-group">
					<input placeholder="Search members" class="form-control" type="text" size="15" value="" name="search[keyword]">
...
{foreach from=$aUsers name=users item=aUser}
	...
	<!-- name_photo_detail -->
	<div class="col-xs-12 col-sm-6 col-md-4 col-lg-3 mac-element" id="js_parent_user_{$aUser.user_id}">
		...
		<div itemscope itemtype="http://schema.org/Person">
			...
			<div class="thumbnail">
				...
				{img user=$aUser suffix='_200_square' class='mac-img-fullwidth'}	

				<div class="caption">
					<div class="user_tooltip_info_user" itemprop="name">
						<h4>{$aUser|user:'':'':50|split:20}</h4>
					</div>
				...
				<div class="user_browse_mutual_friend">
					<a class="btn btn-default btn-xs" href="#" onclick="$Core.box('friend.getMutualFriends', 300, 'user_id={$aUser.user_id}'); return false;">
					{if $aUser.mutual_friends == 1}
				...
				{if Phpfox::isUser() && Phpfox::isModule('friend') && !$aUser.is_friend && $aUser.is_friend_request}
					<div class="alert alert-info mac-mini-alert">
						{phrase var='profile.pending_friend_request'}
					</div>
				{else}
					...
					<div class="user_browse_add_friend">
						{img theme='misc/friend_added.png' class='v_middle'} <a class="btn btn-default btn-xs" href="#" onclick="return $Core.addAsFriend('{$aUser.user_id}');">{phrase var='user.add_friend'}</a>
					</div>
					{/if}
				{/if}
				{/if}
				...
				{if Phpfox::getUserId() && Phpfox::isModule('pinfollow')}
				  {if $aUser.user_id != $aGlobalUser.user_id}
				  <a href="javascript:;" onclick="$Core.box('pinfollow.addFollow', 500, 'following={$aUser.user_id}&follower={$aGlobalUser.user_id}');$(this).hide();$(this).next('.btn').show();return false;" 
				  style='margin:0;' class="mac-mrg-tp btn btn-block btn-default">{phrase var='pinuser.follow'}</a>
				  <a href="javascript:;" onclick="$Core.box('pinfollow.removeFollow', 500, 'following={$aUser.user_id}&follower={$aGlobalUser.user_id}');$(this).hide();$(this).prev('.btn').show();return false;" 
				  style='margin:0;width:90px;display:none' class="mac-mrg-tp btn btn-block btn-danger">{phrase var='pinuser.unfollow'}</a>
				  {else}
				  {/if}
				{/if}
	...
	{else}
		...
		<!-- name_photo -->
			...
			<div class="go_left js_parent_user mac-element" style="{if Phpfox::isMobile()}width:44%;{else}width:120px;{/if} margin-bottom:20px;"  id="js_parent_user_{$aUser.user_id}">	
				<div itemscope itemtype="http://schema.org/Person">
					<meta itemprop="url" content="{url link=$aUser.user_name}" />
...
{else}
	...
	<ul class="action">
		<li><a href="{url link='user.browse'}" class="btn btn-default btn-block">{phrase var='user.reset_browse_criteria'}</a></li>
	</ul>
{/if}
{/if}
</div>
