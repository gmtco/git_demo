{if isset($bIsViewingQuiz)}
	...
	<div>
		{if isset($aQuiz.view_id) && $aQuiz.view_id == 1 && Phpfox::getUserParam('quiz.can_approve_quizzes')}
			...
			<a href="#" class="btn btn-default btn-xs" onclick="$(this).hide(); $('#js_item_bar_approve_image').show(); $.ajaxCall('quiz.approve','iQuiz={$aQuiz.quiz_id}&amp;inline=true'); return false;">{phrase var='quiz.approve'}</a>
		{/if}
		<div class="dropdown">
			<a href="#" class="btn btn-default dropdown-toggle btn-xs" data-toggle="dropdown">
				<i class="icon-cogs"></i> {phrase var='quiz.actions'}
			</a>		
