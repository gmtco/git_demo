...
{/if}
	...
	<div id="js_custom_list_actual_holder">
		<form method="post" action="#" onsubmit="return $Core.updateCustomList(this);">
			...
			<div class="p_top_4">
				<select class="form-control js_custom_list_option" name="custom_list[]" multiple="multiple" size="6" style="width:300px;">
			...
			<div class="p_top_4">
				<input type="submit" class="btn btn-primary" value="{phrase var='privacy.save'}" /> <a href="#" onclick="$('#js_create_custom_friend_list_holder').show(); $('#js_custom_list_actual_holder').hide(); return false;">{phrase var='privacy.or_create_a_new_list'}</a>
			</div>
		</form>
	</div>
		...
		<div id="js_create_custom_friend_list">
			...
			<div style="margin-top:10px;">
				...
				<form method="post" action="#" onsubmit="return false;">
					<div><input class="form-control" type="text" name="name" value="{phrase var='friend.create_new_list'}" maxlength="255" size="15" placeholder="{phrase var='friend.create_new_list' phpfox_squote=true}" id="js_add_new_list" style="vertical-align:middle;" /> 
						<input type="submit" value="{phrase var='friend.add'}" class="btn btn-primary" onclick="if ($('#js_add_new_list').val() != '') $('#js_add_new_list').ajaxCall('friend.addList', 'custom=true'); " /></div>
				</form>		
			</div>
		</div>
				...
				<form method="post" action="#" onsubmit="return false;">
					...
					<div>
						...
						<div id="js_custom_search_submit_button" class="p_top_4 t_right" style="display:none;">
							<input type="button" class="btn btn-primary" value="{phrase var='privacy.save'}" onclick="$(this).parents('form').ajaxCall('friend.addFriendsToList');" />
						</div>
					</div>
					<div class="clear"></div>
				</form>
