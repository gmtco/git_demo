...
<div class="mac-element col-xs-12 col-md-3" id="js_video_id_{$aVideo.video_id}">
	<div class="mac-browsing-video {if isset($aVideo.is_sponsor) && $aVideo.is_sponsor}row_sponsored_image{/if} {if isset($aVideo.is_featured) && $aVideo.is_featured}row_featured_image{/if}">
	...
	<div class="thumbnail">
		...
		<a href="{$aVideo.link}" class="js_video_title_{$aVideo.video_id}">
			...
			<img src="https://c197913.ssl.cf1.rackcdn.com/{$aVideo.vidly_url_id}/thumbnail_1.jpg" alt="{$aVideo.title|clean}" class='mac-img-fullwidth js_mp_fix_width video_image_border' />
			{else}
			{if file_exists(sprintf($aVideo.image_path, '_12090'))}
				{img max_width='200' max_height='500' server_id=$aVideo.image_server_id path='video.url_image' file=$aVideo.image_path suffix='_12090' class='mac-img-fullwidth js_mp_fix_width video_image_border' title=$aVideo.title itemprop='image'}
			{else}
				{img max_width='200' max_height='500' server_id=$aVideo.image_server_id path='video.url_image' file=$aVideo.image_path suffix='_120' class='mac-img-fullwidth js_mp_fix_width video_image_border' title=$aVideo.title itemprop='image'}
			{/if}
			{/if}
		</a>
      ...
      <div class="caption">
        ...
        <h4 itemprop="name">
			<a href="{$aVideo.link}" class="row_sub_link js_video_title_{$aVideo.video_id}" id="js_video_title_{$aVideo.video_id}" itemprop="url">
		...
		{else}
		<div class="badge"{if !$aVideo.is_featured} style="display:none;"{/if}>
			{phrase var='video.featured'}
		</div>
		{/if}
		<div class="badge"{if $aVideo.view_id != 2} style="display:none;"{/if}>
			{phrase var='video.pending'}
		</div>
		<div class="badge"{if !$aVideo.is_sponsor} style="display:none;"{/if}>
			{phrase var='video.sponsored'}
		</div>

		{if !empty($aVideo.duration)}
			<div class="mac_video_duration video_duration">
				{$aVideo.duration}
			</div>
		{/if}
      
		{if Phpfox::getUserParam('video.can_approve_videos') || Phpfox::getUserParam('video.can_delete_other_video')}
		<div class="mac_video_moderate_link video_moderate_link">
			<a href="#{$aVideo.video_id}" class="moderate_link" rel="video">
				{phrase var='video.moderate'}
			</a>
		</div>
		{/if}
