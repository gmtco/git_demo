<div id="js_actual_upload_form">
	{$sCreateJs}
	<form class="form-horizontal mac-form-music-album" method="post" action="{url link='current'}" enctype="multipart/form-data">
		{if $bIsEdit}
		...
		<div id="js_upload_music_detail" class="page_section_menu_holder">
			<div class="table">
				<div class="table_left">
					<label>{required}{phrase var='music.name'}</label>
				</div>
				<div class="table_right">
					<input class="form-control" placeholder="{phrase var='music.name'}" type="text" name="val[name]" size="30" value="{value type='input' id='name'}" id="name" />
				</div>
			</div>
			<div class="table">
				<div class="table_left">
				...
				<div class="table_right">
					<input class="form-control" placeholder="{phrase var='music.year'}" type="text" name="val[year]" size="4" value="{value type='input' id='year'}" id="year" maxlength="4" />
			...
			<div class="table">
				...
				<div class="table_right">
					<textarea class="form-control" placeholder="{phrase var='music.year'}" name="val[text]" id="text" style="height:50px;">{value type='textarea' id='text'}</textarea>
				</div>
			</div>
			...
			<div{if $bIsEdit && !empty($aForms.module_id)} style="display:none;"{/if}>			
				<div class="row">
					<div class="col-lg-6">
						<label>{phrase var='music.privacy'}</label>
			...
			<div class="table_clear">
				<input type="submit" value="{if $bIsEdit}{phrase var='music.update'}{else}{phrase var='music.submit'}{/if}" class="btn btn-primary btn-block" />
			</div>
		</div>		
	</form>
	
	<form class="form-horizontal mac-form-music-upl" method="post" action="{url link='music.upload'}" enctype="multipart/form-data" onsubmit="return startProcess({$sGetJsForm}, false);" id="js_album_form" target="js_upload_frame">
