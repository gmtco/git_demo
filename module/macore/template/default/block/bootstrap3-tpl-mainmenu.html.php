  ...
  <div class="container">
    ...
    <div class="navbar-collapse collapse mac-navbar-main navbar-collapse-top">
      ...
      <ul id="mac-main-menu" class="nav navbar-nav">
          ...
          {if Phpfox::getUserBy('profile_page_id') <= 0}
            ...
            {foreach from=$aMainMenus key=iKey item=aMainMenu name=menu}
              ...
              (isset($aMainMenu.is_force_hidden))
              }
              class="dropdown
              {if isset($aMainMenu.is_selected) && $aMainMenu.is_selected} active{/if}
              {if ($aMainMenu.url == 'apps' && count($aInstalledApps))} explore_apps{/if}"
              {else}
              ...
              <a 
                ...
                class="dropdown-toggle no_ajax_link" data-toggle="dropdown"
                {else}
            ...
            <li class="dropdown">
              <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                {phrase var='core.explore'}
              </a>
              <ul class="dropdown-menu">
                {foreach from=$aAppMenus key=iAppKey item=aAppMenu name=app_menu}
                <li><a href="{url link=$aAppMenu.url}">{phrase var=$aAppMenu.module'.'$aAppMenu.var_name}</a></li>
                {/foreach}
              </ul>
            </li>
            {/if}
      ...
      <ul id="mac-right-menu" class="nav navbar-nav navbar-right">
        ...
        <li id="mac-form-top-search" class="hide">
        ...
        <li>
          <a class="no_ajax_link" href="#" onclick="$(this).tooltip('hide');$('#mac-form-top-search').removeClass('hide');$(this).remove();return false;" data-placement="bottom" data-toggle="tooltip" data-original-title="Search on site" rel="tooltip">
            <i class="icon-search"></i>{if Phpfox::isMobile()} Search...{/if}
          </a>
        </li>
        ...
        {foreach from=$aRightMenus key=iKey item=aMenu}
          <li{if isset($aMenu.children) && count($aMenu.children) && is_array($aMenu.children)} class="dropdown"{if Phpfox::isUser() && $aMenu.url == 'user.setting'} id="mac-user-setting-top-menu"{/if}{/if}>
            ...
            {if $aMenu.url == 'user.setting'} {$aGlobalUser.full_name} {else} {phrase var=$aMenu.module'.'$aMenu.var_name}{if isset($aMenu.suffix)}{$aMenu.suffix}{/if}{if isset($aMenu.children) && count($aMenu.children) && is_array($aMenu.children)}{/if}   {/if}
            </a>          
            {if isset($aMenu.children) && count($aMenu.children) && is_array($aMenu.children)}
            <ul class="dropdown-menu">
              {if Phpfox::isUser() && $aMenu.url == 'user.setting'}
                <li class="header_menu_user_link mac-ajaxify-links">                          
                  <div id="header_menu_user_image">
                    {img user=$aGlobalUser suffix='_50_square' max_width=50 max_height=50}
                  </div>
                  <div id="header_menu_user_profile">
