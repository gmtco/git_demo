...
{/if}
	<div id="js_event_block_detail" class="js_event_block page_section_menu_holder">
		...
		<div class="table">
			...
			<div class="table_right">
				<input placeholder="{phrase var='event.what_are_you_planning'}" class="form-control" type="text" name="val[title]" value="{value type='input' id='title'}" id="title" size="40" maxlength="100" />
			</div>
		</div>		
		...
		<div class="table">
			...
			<div class="table_right">
				<input class="form-control" placeholder="{phrase var='event.location_venue'}" type="text" name="val[location]" value="{value type='input' id='location'}" id="location" size="40" maxlength="200" />
				{if !$bIsEdit}
				<div class="extra_info">
		...
		<div id="js_event_add_country"{if !$bIsEdit} style="display:none;"{/if}>	
			...
			<div class="table">
				...
				<div class="table_right">
					<input class="form-control" placeholder="{phrase var='event.address'}" type="text" name="val[address]" value="{value type='input' id='address'}" id="address" size="30" maxlength="200" />
				</div>
			</div>			 			 
			...
			<div class="table">
				...
				<div class="table_right">
					<input class="form-control" placeholder="{phrase var='event.city'}" type="text" name="val[city]" value="{value type='input' id='city'}" id="city" size="20" maxlength="200" />
				</div>
			</div>		
			...
			<div class="table">
				...
				<div class="table_right">
					<input placeholder="{phrase var='event.zip_postal_code'}" class="form-control" type="text" name="val[postal_code]" value="{value type='input' id='postal_code'}" id="postal_code" size="10" maxlength="20" />
				</div>
			</div>		
		...
		<div class="row">
			...
			<div class="col-lg-6 col-12">
				<label>{phrase var='event.event_privacy'}</label>
			...
			<div class="col-lg-6 col-12">
				<label>{phrase var='event.share_privacy'}</label>
		...
		{if $bIsEdit}
			<input type="submit" value="{phrase var='event.update'}" class="btn btn-primary btn-block" />
		{else}	
			<input type="submit" value="{phrase var='event.submit'}" class="btn btn-primary btn-block" />
		{/if}
		</div>
	...
	<div id="js_event_block_customize" class="js_event_block page_section_menu_holder" style="display:none;">
		<div id="js_event_block_customize_holder">
			...
			<div id="js_submit_upload_image" class="table_clear"{if $bIsEdit && !empty($aForms.image_path)} style="display:none;"{/if}>
				<input type="submit" value="{phrase var='event.upload_photo'}" class="btn btn-primary btn-block" />
			</div>
		</div>	
	</div>
	...
	<div id="js_event_block_invite" class="js_event_block page_section_menu_holder" style="display:none;">	
			...
			<h3 style="margin-top:0px; padding-top:0px;">{phrase var='event.invite_friends'}</h3>
				...
				<div class="p_4">
					<textarea class="form-control" name="val[emails]"></textarea>
					<div class="alert alert-info mac-mrg-tp">
						<a class="close" href="#" data-dismiss="alert">&times;</a>
				...
				<div class="p_4">
					<textarea class="form-control" name="val[personal_message]"></textarea>					
				</div>				
				
				<div class="p_top_8">
					<input type="submit" id="btn_invitations_submit" value="{phrase var='event.send_invitations'}" class="btn btn-primary btn-block" />
				</div>				
			</div>
			{if Phpfox::isModule('friend')}
	...
	<div id="js_event_block_email" class="js_event_block page_section_menu_holder" style="display:none;">
		<div id="js_send_email"{if !$bCanSendEmails} style="display:none;"{/if}>
			...
			<div class="table">
				<div class="table_left">
					<label>{phrase var='event.subject'}:</label>
				</div>
				<div class="table_right">
					<input placeholder="{phrase var='event.subject'}" class="form-control" type="text" name="val[mass_email_subject]" value="" size="30" id="js_mass_email_subject" />
				</div>
			</div>
			<div class="table">
				<div class="table_left">
					<label>{phrase var='event.text'}:</label>
				</div>
				<div class="table_right">
					<textarea placeholder="{phrase var='event.text'}" class="form-control" name="val[mass_email_text]" id="js_mass_email_text"></textarea>
				</div>
			</div>		
			<div class="table_clear">
				<ul class="table_clear_button">
					...
					<li>
						<input type="button" value="{phrase var='event.send'}" class="btn btn-primary" onclick="$('#js_event_mass_mail_li').show(); $.ajaxCall('event.massEmail', 'type=message&amp;id={$aForms.event_id}&amp;subject=' + $('#js_mass_email_subject').val() + '&amp;text=' + $('#js_mass_email_text').val()); return false;" />
					</li>
