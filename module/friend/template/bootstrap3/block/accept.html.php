			...
			<div class="drop_data_content">
				<div class="drop_data_user">
					<div class="drop_data_action">
						...
						<div class="js_drop_data_button" id="drop_down_{$aFriend.request_id}">
							<ul class="table_clear_button">
								<li><input type="button" name="" value="{phrase var='friend.confirm'}" class="btn btn-xs btn-primary" onclick="$(this).parents('.drop_data_action').find('.js_drop_data_add').show(); {if $aFriend.relation_data_id > 0} $.ajaxCall('custom.processRelationship', 'relation_data_id={$aFriend.relation_data_id}&amp;type=accept&amp;request_id={$aFriend.request_id}'); {else} $.ajaxCall('friend.processRequest', 'type=yes&amp;user_id={$aFriend.user_id}&amp;request_id={$aFriend.request_id}&amp;inline=true'); {/if}" /></li>
								<li><input type="button" name="" value="{phrase var='friend.deny'}" class="btn btn-xs btn-default" onclick="$(this).parents('.drop_data_action').find('.js_drop_data_add').show(); {if $aFriend.relation_data_id > 0} $.ajaxCall('custom.processRelationship', 'relation_data_id={$aFriend.relation_data_id}&amp;type=deny&amp;request_id={$aFriend.request_id}'); {else} $.ajaxCall('friend.processRequest', 'type=no&amp;user_id={$aFriend.user_id}&amp;request_id={$aFriend.request_id}&amp;inline=true'); {/if}" /></li>
							</ul>
							<div class="clear"></div>
						</div>
					</div>
					<div style="width:120px;">
						...
						{if $aFriend.relation_data_id > 0}
                            <div class="extra_info_link">
                               <i class="icon-heart"></i> {phrase var='friend.relationship_request'}
                            </div>
						{else}
					...
					<ul class="extra_info_middot" style="display:none;">
						<li>
            <a href="#" onclick="$Core.composeMessage({l}user_id: {$aFriend.user_id}{r}); return false;">
              {phrase var='friend.send_a_message'}
            </a>
            </li>
						<li>&middot;</li>
						<li>
            <a href="{url link=$aFriend.user_name}">
              {phrase var='friend.view_profile'}
            </a>
            </li>
					</ul>					
				</div>				
...
{/if}
<a href="{url link='friend.accept'}" class="holder_notify_drop_link">
{phrase var='friend.see_all_friend_requests'}
</a>
{/if}