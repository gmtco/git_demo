...
{/if}
<div><input type="hidden" name="val[user_id]" value="{$aUser.user_id}" /></div>
...
{else}
	...
	<div class="p_top_8">
		<div id="js_personal_link" class="extra_info_link"><a href="#" onclick="$('#js_personal_link').hide(); $('#js_personal_message').show(); return false;">{phrase var='friend.add_a_personal_message'}</a></div>
		<div id="js_personal_message" style="display:none;">
			{phrase var='friend.add_a_personal_message_form'}: <a href="#" onclick="$('#js_personal_message').hide(); $('#js_personal_link').show(); return false;">{phrase var='friend.cancel'}</a>
			<div>
				<textarea placeholder="{phrase var='friend.add_a_personal_message_form'}" id="js_message" class="form-control col-lg-3 mac-mrg-tp mac-mrg-btm" name="val[text]" onkeyup="limitChars('js_message', 250, 'js_limit_info');"></textarea>
				<hr class="invisible">
	...
	<div class="p_top_8" id="container_submit_friend_request">
		<input type="submit" onclick="{if $bSuggestion}$('#js_friend_suggestion').hide(); $('#js_friend_suggestion_loader').show(); {/if}$('#js_process_image').show(); $('#js_process_request').ajaxCall('friend.addRequest');" value="{phrase var='friend.add_friend'}" class="btn btn-primary" /> 
		<span id="js_process_image" style="display:none;"><i class="icon-spinner icon-spin icon-2x"></i></span>
	</div>
{/if}
</div>
<div class="clear"></div>
