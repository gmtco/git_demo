...
{if $sFriendType != 'mail'}
	<div class="row">
		<div class="col-lg-4 col-md-4 col-xs-12">
				<select class="form-control" name="view" onchange="showLoader(); $(this).ajaxCall('friend.searchAjax', 'input={$sPrivacyInputName}'); return false;">
		...
		<div class="col-lg-8 col-md-8 col-xs-12">
			...
			<div class="input-group">
				<input class="form-control" placeholder="{phrase var='friend.search_by_email_full_name_or_user_name'}" type="text" class="js_is_enter v_middle default_value" name="find" value="{phrase var='friend.search_by_email_full_name_or_user_name'}" onfocus="if (this.value == sSearchByValue){literal}{{/literal}this.value = ''; $(this).removeClass('default_value');{literal}}{/literal}" onblur="if (this.value == ''){literal}{{/literal}this.value = sSearchByValue; $(this).addClass('default_value');{literal}}{/literal}" id="js_find_friend" size="30" />
				<span class="input-group-btn">
					<input type="button" value="{phrase var='friend.find'}" onclick="showLoader(); $.ajaxCall('friend.searchAjax', 'friend_module_id={$sFriendModuleId}&amp;friend_item_id={$sFriendItemId}&amp;find=' + $('#js_find_friend').val() + '&amp;input={$sPrivacyInputName}'); return false;" class="btn btn-primary" />
				</span>
			</div>
		</div>
		<div class="clear"></div>
...
{if !$bSearch}
	...
	<div class="main_break t_right">		
		<input type="button" name="submit" value="{phrase var='friend.use_selected'}" onclick="{literal}if (function_exists('plugin_selectSearchFriends')) { plugin_selectSearchFriends(); } else { $Core.loadInit(); tb_remove(); }{/literal}" class="btn btn-primary" />

		<input type="button" name="cancel" value="{phrase var='friend.cancel'}" onclick="{literal}if (function_exists('plugin_cancelSearchFriends')) { plugin_cancelSearchFriends(); } else { cancelFriendSelection(); }{/literal}" class="btn btn-default" />		
	</div>
	{/if}
	{/if}
</div>
