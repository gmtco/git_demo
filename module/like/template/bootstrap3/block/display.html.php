{if Phpfox::getParam('like.show_user_photos')}
	<div class="activity_like_holder comment_mini" style="position:relative;">		
		<a data-placement="left" data-toggle="tooltip" rel="tooltip" data-original-title="{if defined('PHPFOX_IS_THEATER_MODE')}{phrase var='like.likes'}{else}{phrase var='like.people_who_like_this'}{/if}" href="#" class="like_count_link" onclick="return $Core.box('like.browse', 400, 'type_id={if isset($aFeed.like_type_id)}{$aFeed.like_type_id}{else}{$aFeed.type_id}{/if}&amp;item_id={$aFeed.item_id}');">			
			<i class="icon-thumbs-up"></i> {$aFeed.feed_total_like|number_format}
		</a>		
	...
	{if isset($aFeed.feed_like_phrase) }
		{if !empty($aFeed.feed_like_phrase)}{* Has to be like this so it can also display nothing *}
			<div class="activity_like_holder comment_mini" id="activity_like_holder_{$aFeed.feed_id}">
				<i class="icon-thumbs-up"></i> {$aFeed.feed_like_phrase}
			</div>
			{else} 
		{/if}
