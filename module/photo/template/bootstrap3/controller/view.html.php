...
<div class="alert alert-info js_moderation_off">
		...
		<div class="photo_view_box_comment_padding">
			...
			<div id="js_photo_view_box_title">
				...
				<div class="">
					...
					<a href="#" class="item_bar_approve_image" onclick="return false;" style="display:none;" id="js_item_bar_approve_image">
						<i class="icon-spinner icon-spin"></i>
					</a>
					<a href="#" class="btn btn-default btn-xs" onclick="$(this).hide(); $('#js_item_bar_approve_image').show(); $.ajaxCall('photo.approve', 'inline=true&amp;id={$aForms.photo_id}'); return false;">
						{phrase var='photo.approve'}
					</a>
					{/if}
					<div class="dropdown">
						...
						<ul class="dropdown-menu mac-no-ajaxify-links">
...
$Behavior.macReloadPhoto = function(){l}
	$(window).on('resize', function(){l}
		$Behavior.autoLoadPhoto();
	{r});
{r}
...
<div class="item_view photo_item_view" {if $bIsTheater} id="photo_view_theater_mode"{/if}>
	<div id="js_album_outer_content">
	    ...
	    <div class="item_info">
			...
			{if !empty($aForms.album_id)} <br /> {phrase var='photo.in'} 
			<a href="{$aForms.album_url}">
				{$aForms.album_title|clean|split:45|shorten:75:'...'}
			</a>
			{/if}
	    </div>
	    {/if}
		...
		{if !$bIsTheater}<hr class="invisible">{/if}
		...
		<div id="js_photo_view_main_holder"{if !Phpfox::isMobile()} style="margin-bottom:30px;"{/if}>
		    ...
		    <div class="photo_next_previous">
				...
				{if isset($aPhotoStream.previous.photo_id)}
				<li class="previous">
					<a href="{$aPhotoStream.previous.link}{if $iForceAlbumId > 0}albumid_{$iForceAlbumId}{else}{if isset($feedUserId)}userid_{$feedUserId}/{/if}{/if}"{if $bIsTheater} class="thickbox photo_holder_image" rel="{$aPhotoStream.previous.photo_id}"{/if}>
	    ...
	    <div class="photo_next_previous">
			...
			{if isset($aPhotoStream.previous.photo_id)}
			<li class="previous">
				<a class="btn btn-default btn-xs" href="{$aPhotoStream.previous.link}{if $iForceAlbumId > 0}albumid_{$iForceAlbumId}{else}{if isset($feedUserId)}userid_{$feedUserId}/{/if}{/if}">
					<i class="icon-chevron-left"></i> {phrase var='photo.previous'}
				</a>
			</li>
			{/if}	
			...
			{if isset($aPhotoStream.next.photo_id)}
			<li class="next">
				<a class="btn btn-default btn-xs" href="{$aPhotoStream.next.link}{if $iForceAlbumId > 0}albumid_{$iForceAlbumId}{else}{if isset($feedUserId)}userid_{$feedUserId}/{/if}{/if}">
					{phrase var='photo.next'} <i class="icon-chevron-right"></i>
				</a>
			</li>
			{/if}
			</ul>
		...
		<div class="photo_view_detail{if !$bIsTheater} col-lg-2 col-md-2 col-xs-12{/if}">			
			{module name='photo.detail'}
		</div>	

		<div class="photo_view_comment{if !$bIsTheater} {if !Phpfox::isMobile() && isset($aBlockAds) && count($aBlockAds)}col-lg-8 col-md-8{else}col-lg-10 col-md-10{/if}{/if} col-xs-12">
		{/if}
			{if $aForms.description}
		...
		{if !Phpfox::isMobile()}
			...
			{if !Phpfox::isMobile() && isset($aBlockAds) && count($aBlockAds)}

				<div class="photo_view_ad{if !$bIsTheater} col-lg-2 col-md-2 col-xs-12{/if}">
					{module name='ad.display' block_id='photo_theater'}
				</div>
