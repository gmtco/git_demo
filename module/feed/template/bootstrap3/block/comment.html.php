{if 
	!Phpfox::getUserId() && 
	Phpfox::getParam('macore.mac_use_disqus') &&
	isset($sFeedType) &&
	$sFeedType == 'view'
} 

{template file='macore.block.disqus'}
		...
		{else}
		    {if isset($aFeed.comment_type_id) && isset($aFeed.total_comment) && (isset($sFeedType) &&  $sFeedType == 'mini' ? $aFeed.total_comment > 0 : $aFeed.total_comment > Phpfox::getParam('comment.total_comments_in_activity_feed'))}
			    <div class="comment_mini comment_mini_link_holder" id="js_feed_comment_view_more_link_{$aFeed.feed_id}">
				    ...
				    <div class="comment_mini_link_loader" id="js_feed_comment_ajax_link_{$aFeed.feed_id}" style="display:none;">
				    	<i class="icon-spinner icon-spin"></i>
				    </div>
			...
			<div class="{if isset($sFeedType) &&  $sFeedType == 'view'} feed_item_view{/if} comment_mini comment_mini_end">
				<form method="post" action="#" class="js_comment_feed_form">				
					...
					<div class="{if isset($sFeedType) &&  $sFeedType == 'view'}comment_mini_content {/if}comment_mini_textarea_holder">
						...
						<textarea placeholder="{phrase var='feed.write_a_comment'}" name="val[text]" class="js_comment_feed_textarea form-control input-lg" id="js_feed_comment_form_textarea_{$aFeed.feed_id}">{if isset($sFeedType) &&  $sFeedType == 'view' && Phpfox::getUserParam('comment.wysiwyg_on_comments') && Phpfox::getParam('core.wysiwyg') == 'tiny_mce'}{else}{phrase var='feed.write_a_comment'}{/if}</textarea>

					</div>
					<div class="feed_comment_buttons_wrap" style="display:block;">
						<div class="js_feed_add_comment_button t_right">
							<input data-loading-text="{phrase var='feed.adding_your_comment'}..." type="submit" value="{phrase var='feed.comment'}" class="mac-btn-loading btn btn-primary btn-xs" />														
						</div>								
					</div>			
