...
{else}
	...
	{if Phpfox::isUser() && !defined('PHPFOX_IS_USER_PROFILE') && !PHPFOX_IS_AJAX && !defined('PHPFOX_IS_PAGES_VIEW')}
		<div class="feed_sort_order">
			<a class="feed_sort_order_link dropdown-toggle no_ajax_link" data-toggle="dropdown" href="#">
				<i class="icon-sort-by-attributes"></i> {phrase var='feed.sort'}
			</a>
			<ul class="dropdown-menu feed_sort_holder pull-right" style="margin-top: 10px;">
				<li{if !$iFeedUserSortOrder} class="active"{/if}>
					<a href="#" rel="0">
						<i class="icon-star"></i> {phrase var='feed.top_stories'}
					</a>
				</li>
				<li{if $iFeedUserSortOrder} class="active"{/if}>
					<a href="#" rel="1">
						<i class="icon-clock-o"></i> {phrase var='feed.most_recent'}
					</a>
				</li>
			</ul>
		</div>
	...
	{if !PHPFOX_IS_AJAX && (Phpfox::getParam('macore.use_2_column_wall_layout') || (Phpfox::isModule('pin') && Phpfox::getParam('pin.use_2_column_wall')))}
	<div class="btn-group btn-group-justified mac-mrg-btm">
		<a href="javascript:void(0);" id="js-make-2col" class="disabled btn btn-link mac-col-switcher-link">
			<i class="icon-th-large"></i> Switch to two column
		</a>
		<a href="javascript:void(0);" id="js-make-1col" class="btn btn-link mac-col-switcher-link">
			<i class="icon-align-justify"></i> Switch to one column
		</a>
	</div>
	{/if}

	{if Phpfox::getParam('feed.refresh_activity_feed') > 0 && Phpfox::getLib('module')->getFullControllerName() == 'core.index-member'}
		<div id="activity_feed_updates_link_holder" class="mac-mrg-btm">
			<a href="#" id="activity_feed_updates_link_single" class="activity_feed_updates_link btn btn-block btn-primary" onclick="return $Core.loadMoreFeeds();">{phrase var='feed.1_new_update'}</a>
			<a href="#" id="activity_feed_updates_link_plural" class="activity_feed_updates_link btn btn-block btn-primary" onclick="return $Core.loadMoreFeeds();">{phrase var='feed.span_id_js_new_update_view_span_new_updates'}</a>
		</div>
	{/if}
...
<div {if !$bIsHashTagPop}id="js_feed_content"{/if}>
	...
	{else}	
		...
		{else}
			...
			{if Phpfox::getParam('macore.use_2_column_wall_layout') || (Phpfox::isModule('pin') && Phpfox::getParam('pin.use_2_column_wall'))}
				<div class="mac-isotope2-wrapper">
				<div class="mac-feed-isotope">
			{/if}
			...
			{foreach from=$aFeeds name=iFeed item=aFeed}
				...
				<div {if Phpfox::getParam('macore.use_2_column_wall_layout') || (Phpfox::isModule('pin') && Phpfox::getParam('pin.use_2_column_wall'))}class="col-xs-12 col-md-6 mac-feed-box"{/if}>
				<div class="feed-type-{$aFeed.type_id} js_feed_view_more_entry_holder">
				...
				{if isset($aFeed.more_feed_rows) && is_array($aFeed.more_feed_rows) && count($aFeed.more_feed_rows) && (Phpfox::getParam('macore.use_2_column_wall_layout') || (Phpfox::isModule('pin') && Phpfox::getParam('pin.use_2_column_wall')))}
					{foreach from=$aFeed.more_feed_rows item=aFeed}
						{if $bChildFeed = true}{/if}
						<div class="col-xs-12 col-md-6 mac-feed-box ">
							<div class="feed-type-{$aFeed.type_id} js_feed_view_more_entry_holder">
								{template file='feed.block.entry'}
							</div>
						</div>
	...
	{if $sCustomViewType === null}
		...
		{else}
			...
			{else}
				...
				{else}
					<br />
					<div class="alert alert-info js_no_feed_to_show">{phrase var='feed.there_are_no_new_feeds_to_view_at_this_time'}</div>
				{/if}
			{/if}
		{/if}
	{/if}
	...
	{if Phpfox::getService('profile')->timeline()}
		...
		{if isset($aUser.page_user_id)}
			<div id="mac-right" class="mac-fix-phpfox-messed-code animated fadeInRight col-xs-12 col-sm-9 col-md-3 col-lg-3 col-1-{$sMacBodyClass}">
