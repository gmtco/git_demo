<div class="activity_feed_form_share">
	...
	<div class="activity_feed_form_share_process">
	...
	{if !isset($bSkipShare)}
		<ul class="activity_feed_form_attach">
			...
			<li>
				<a href="#" rel="global_attachment_status" class="active">
					<i class="icon-comment icon-2x"></i> {*phrase var='feed.post'*}
			...
			<li>
				<a href="#" rel="global_attachment_status" class="active">
					<i class="icon-comment icon-2x"></i> {*phrase var='feed.status'*}
					<span class="activity_feed_link_form_ajax">user.updateStatus</span>
					<div class="drop"></div>
				</a>
			</li>
			...
			<li>
				<a href="#" rel="global_attachment_status" class="active">
					...
					<span class="activity_feed_link_form_ajax">feed.addComment</span>
					<div class="drop"></div>
				</a>
			</li>
			...
			{if $phpfox.iteration.feedlinks == 10 && Phpfox::getService('profile')->timeline()}
			<li>
				<a href="#" rel="view_more_link" class="timeline_view_more js_hover_title">
					<span class="js_hover_info">{phrase var='feed.view_more'}</span>
				</a>
				...
				{if ($aFeedStatusLink.no_profile && !isset($bFeedIsParentItem) && (!defined('PHPFOX_IS_USER_PROFILE') || (defined('PHPFOX_IS_USER_PROFILE') && isset($aUser.user_id) && $aUser.user_id == Phpfox::getUserId()))) || !$aFeedStatusLink.no_profile}
					<li>
						<a href="#" rel="global_attachment_{$aFeedStatusLink.module_id}"{if $aFeedStatusLink.no_input} class="no_text_input"{/if}>
							<i class="icon-{$aFeedStatusLink.module_id} icon-2x"></i>
	...
	<form method="post" action="#" id="js_activity_feed_form" enctype="multipart/form-data">
		...
		<div class="activity_feed_form_holder">		
			...
			<div class="global_attachment_holder_section" id="global_attachment_status" style="display:block;">
				...
				<textarea placeholder="{if isset($aFeedCallback.module) || defined('PHPFOX_IS_USER_PROFILE')}{phrase var='feed.write_something'}{else}{phrase var='feed.what_s_on_your_mind'}{/if}" class="mac-share-txt form-control input-lg" {if isset($aPage)} id="pageFeedTextarea" {else} {if isset($aEvent)} id="eventFeedTextarea" {else} {if isset($bOwnProfile) && $bOwnProfile == false}id="profileFeedTextarea" {/if}{/if}{/if} cols="60" name="val[user_status]"></textarea>

                {if isset($bLoadCheckIn) && $bLoadCheckIn == true}
                    <script type="text/javascript">
                        oTranslations['feed.at_location'] = "<i class='icon-map-marker'></i>";
                    </script>
		...
		<div class="activity_feed_form_button">
			...
			<div class="activity_feed_form_button_status_info">
				<textarea placeholder="{phrase var='macore.mac_placeholder_start_writing'}" id="activity_feed_textarea_status_info" class="form-control" name="val[status_info]"></textarea>
			</div>
			<div class="activity_feed_form_button_position">
				
				{if ((defined('PHPFOX_IS_PAGES_VIEW') && $aPage.is_admin) || ((Phpfox::isModule('share') && !defined('PHPFOX_IS_USER_PROFILE') && !defined('PHPFOX_IS_PAGES_VIEW') && !defined('PHPFOX_IS_EVENT_VIEW') && (Phpfox::getParam('share.share_on_facebook') || Phpfox::getParam('share.share_on_twitter'))) || (defined('PHPFOX_IS_USER_PROFILE') && isset($aUser.user_id) && $aUser.user_id == Phpfox::getUserId() && Phpfox::getService('profile')->timeline() && Phpfox::getParam('feed.can_add_past_dates'))))}
					...
					<div id="activity_feed_share_this_one">
						<ul>
							{if (Phpfox::isModule('share') && !defined('PHPFOX_IS_USER_PROFILE') && !defined('PHPFOX_IS_PAGES_VIEW') && !defined('PHPFOX_IS_EVENT_VIEW') && (Phpfox::getParam('share.share_on_facebook') || Phpfox::getParam('share.share_on_twitter')))}
				              <li>
				              	<a href="#" data-placement="top" data-original-title="{phrase var='feed.share_this_on'}" class="mac-tooltip activity_feed_share_this_one_link parent feed_share_site" rel="feed_share_on_holder">
							...
							<li>
								<div class="parent">
									<select class="form-control" name="custom_pages_post_as_page">
				...
				<div class="timeline_date_holder_share timeline_date_holder">					
					<div class="t_center p_top_8"><i class="icon-spinner icon-spin icon-2x"></i></div>					
				</div>
				{/if}				
				
				<div class="activity_feed_form_button_position_button">
					<input type="submit" value="{phrase var='feed.share'}" id="activity_feed_submit" class="btn btn-primary{if Phpfox::isMobile()} btn-sm{/if}" />
				</div>				
				{if isset($aFeedCallback.module)}
				{else}
